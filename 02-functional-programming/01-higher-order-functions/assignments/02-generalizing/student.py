...
def sum_movie_duration_in_period(movies, period):
    total_duration = 0
    for movie in movies:
        if movie.year >= period[0] and movie.year < period[1]:
...
def find_number_greater_than(numbers, treshold):
    for number in numbers:
        if number > treshold:
            return number
    return None

print(count_movies_from_year(movies, 2013))
print(select_movies_with_actor(movies, "Leonardo DiCaprio"))
print(sum_movie_duration_in_period(movies, (1990, 2000)))
print(get_directors_of_actor(movies, "Leonardo DiCaprio"))

print(find_string_starting_with(movies[2].actors, 'a'))
runtime_list = []
for movie in movies:
    runtime_list.append(movie.runtime)
print(find_number_greater_than(runtime_list, 180))